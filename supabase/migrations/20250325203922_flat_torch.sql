...
DROP FUNCTION IF EXISTS check_user_exists_v2;
DROP FUNCTION IF EXISTS update_user_profile_v2;
...
-- Create update_user_profile with proper parameter handling
...
REVOKE ALL ON FUNCTION update_user_profile(text, boolean, uuid) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION update_user_profile(text, boolean, uuid) TO authenticated;

-- Add helpful comments
COMMENT ON FUNCTION check_user_exists_v2(text) IS 'JSON wrapper for checking if a user exists';
COMMENT ON FUNCTION update_user_profile(text, boolean, uuid) IS 'Updates a user''s profile information';