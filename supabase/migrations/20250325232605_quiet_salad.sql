/*
  ...
  1. Changes
    - Drop all existing user profile functions
    - Create new functions with unique names and proper parameter handling
...
REVOKE ALL ON FUNCTION update_user_profile_rpc(text, boolean, uuid) FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION update_user_profile_rpc(text, boolean, uuid) TO authenticated;
...
COMMENT ON FUNCTION update_user_profile_rpc(text, boolean, uuid) IS 'Updates a user''s profile information';