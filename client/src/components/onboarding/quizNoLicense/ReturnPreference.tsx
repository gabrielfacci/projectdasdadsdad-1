...
import { TrendingUp, Check, Sparkles, BarChart, ArrowUp, ArrowDown } from 'lucide-react';
import { animations } from './onboardingConfig';
import { useTranslation } from '../../../hooks/useTranslation';
import '../../../index.css';
import './onboardingResponsive.css';

interface ReturnPreferenceProps {
  onComplete: (preference: string) => void;
}

const ReturnPreference: React.FC<ReturnPreferenceProps> = ({ onComplete }) => {
  const { t } = useTranslation();
  const [selectedPreference, setSelectedPreference] = useState<string | null>(null);

  const handleSelect = (preference: string) => {
    setSelectedPreference(preference);
    setTimeout(() => onComplete(preference), 500);
  };

  const preferences = [
    {
      value: "high_risk",
      label: t('onboarding.returnPreference.highRisk.title'),
      description: t('onboarding.returnPreference.highRisk.description'),
      icon: ArrowUp,
      color: '#FF6B6B'
    },
    {
      value: "balanced",
      label: t('onboarding.returnPreference.balanced.title'),
      description: t('onboarding.returnPreference.balanced.description'),
      icon: BarChart,
      color: '#FFD166'
    },
    {
      value: "low_risk",
      label: t('onboarding.returnPreference.lowRisk.title'),
      description: t('onboarding.returnPreference.lowRisk.description'),
      icon: ArrowDown,
      color: '#4ECDC4'
    }
  ];
  ...
  return (
    ...
    >
      ...
      <motion.h2
        ...
        variants={itemVariants}
      >
        {t('onboarding.returnPreference.title')}
      </motion.h2>
      ...
      <motion.p
        ...
        variants={itemVariants}
      >
        {t('onboarding.returnPreference.subtitle')}
      </motion.p>
      ...
      >
        {preferences.map((preference, index) => (
          <motion.div
            ...
            initial="hidden"
            animate={selectedPreference === preference.value ? "selected" : "visible"}
            whileHover={selectedPreference !== preference.value ? "hover" : {}}
            whileTap="tap"
          ...
          >
            ...
            <div className="flex items-center gap-3 sm:gap-4 relative z-10">
              ...
              <div className="bg-background-light/20 p-2 rounded-lg">
                <preference.icon className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
              </div>

              {/* Level info */}
              <div className="flex-1">
                <h3 className="text-sm sm:text-base font-medium text-white">{preference.label}</h3>
                <p className="text-[14px] text-neutral-400 mt-0.5">{preference.description}</p>
              </div>
            ...
            {selectedPreference === preference.value && (
              <motion.div 
                ...
                style={{ backgroundImage: `linear-gradient(to right, ${preference.color}30, transparent)` }}
                initial={{ opacity: 0 }}
                animate={{ opacity: 0.1 }}
                exit={{ opacity: 0 }}
              />
            ...
            <motion.div 
              ...
              whileHover={{ 
                opacity: 1,
                borderColor: `${preference.color}50`,
              }}
      ...
      {!selectedPreference && (
        <motion.div
          ...
          variants={itemVariants}
        >
          <p>{t('onboarding.returnPreference.selectOption')}</p>
        </motion.div>
      )}
